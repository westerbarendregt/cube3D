...
/*   Updated: 2020/02/26 17:07:14 by wbarendr      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void			make_world_no(t_struct *w, float distance, int x)
{
	...
	while (i <= w->wl->end)
	{
		...
		pixelput_three_d(w, x, w->wl->eq + i,
		make_shade((*(unsigned int*)color), distance));
...
void			make_sprite(t_struct *w, float distance, float on_wall, int x)
{
	...
	while (i <= w->wl->end)
	{
		...
		if (*(unsigned int*)cl != 0x00000000)
			pixelput_three_d(w, x, w->wl->eq + i,
			make_shade((*(unsigned int*)cl), distance));
